		...
		try{
			...
			for (File fl : pilhaArqs) {
				...
				for (int i = 1; i <= 5; i++) {
					...
					ImageIO.write(outImg, "JPG", new File(dir+"pyramidFloat\\"+fl.getName()+".jpg"));
				}
				...
				for (int i = 1; i <= 3; i++) {
					...
					bufferedWriter.write("\n Dimensão pyramid: "+outImg.getWidth()+" x "+outImg.getHeight()+"\n");				
					System.out.println(((System.currentTimeMillis() - start)/1000.0)+" segundos");
					ImageIO.write(outImg, "JPG", new File(dir+"pyramidDiscrete\\"+fl.getName()+".jpg"));
				}
				bufferedWriter.close();
				flWriter.close();
			}	
