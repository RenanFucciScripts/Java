...
public class HaarFilter {
	...
	public byte[] invert(int[] filter, byte[] values) {
		...
		//lazily allocate a buffer
		if (buffer == null) buffer = new int[values.length];
	...
	public static void main(String[] args) throws IOException {
		HaarFilter haar =  new HaarFilter(512);
		haar.setFractionalBits(0);
		String dir = "C:\\Users\\Renan Fucci\\Desktop\\ImgTeste\\";
		byte[] data = BufferedImageToBytes.bufferedImageToBytes(dir+"img.png"); 
		int[] filter = haar.filter(data, null);
		BufferedImage forward = haar.filterToImage(filter);
		ImageIO.write(forward, "PNG", new File(dir+"filter.png"));
		System.out.println("Filter: "+filter.length);
		System.out.println("Data: "+data.length);

		byte[] invert =haar.invert(filter, data);
		BufferedImage reverse = haar.valuesToImage(invert);
		ImageIO.write(reverse, "PNG",new File(dir+"reverse.png"));
	}
