...
public class MétodosPDI {
	 ...
	 */
	public static void gravarImagem(int matriz[][], String dir, String nomeArq_Extns){
		...
		try{
			...
			ImageIO.write(imagem, "BMP",new File(dir+nomeArq_Extns));
	 ...
	 */
	public static void gravarImagemColorida(int matrizRed[][], int matrizGreen[][], int matrizBlue[][], String dir, String nomeArq_Extens){
		...
		try{
			...
			//Escreve a imagem em um diretório
			ImageIO.write(imagem, "JPG", new File(dir+nomeArq_Extens));
		}
		//Captura exceções
		catch(Exception ex){
			//Imprime a exceção
	...
	public static int[][] codificarMsgBinária(int[][] matrizOriginal, String msg){
		...
		for (char c: msg.toCharArray()) {  
			...
			for(char d:letra.toCharArray()){
				//System.out.println("Char.to: "+Character.toString(d));
		...
		//System.out.println(matrizMsg.length+" "+matrizresult7[0].length);
		for (int i = 0; i < matrizresult7.length; i++) {
			for (int j = 0; j < matrizresult7[i].length; j++) {
				if(vet.size()==cont){
					...
					//System.out.println("vetGet: "+vet.get(cont));
					
					matrizMsg[i][j]=Integer.parseInt(vet.get(cont));
					cont+=1;
					//System.out.println("mmint: "+matrizMsg[i][j]);
