...
public class TesteMLP {
		...
		try {
			...
			while(true){
				if(cont==0){
					if(linha1==null){
						...
						if(indices.contentEquals("NDWI, EVI")==true){
							double[] vet ={evi,ndwi};
							trainingSet1.addRow (new DataSetRow (vet, new double[]{0,0,0}));
						}
				...
				if(cont==1){
					...
					if(linha2!=null){
						...
						if(indices.contentEquals("NDWI, EVI")==true){
							double[] vet ={evi,ndwi};
							trainingSet1.addRow (new DataSetRow (vet, new double[]{1,1,1}));
						}
						else if(indices.contentEquals("NDVI, EVI, NIR")==true){
							double[] vet ={evi,ndvi, nir};
							trainingSet1.addRow (new DataSetRow (vet, new double[]{1,1,1}));
					...
					if(linha3!=null){
						...
						if(indices.contentEquals("NDWI, EVI")==true){
							double[] vet ={evi,ndwi};
							trainingSet1.addRow (new DataSetRow (vet, new double[]{2,2,2}));
						}
						else if(indices.contentEquals("NDVI, EVI, NIR")==true){
							double[] vet ={evi,ndvi, nir};
							trainingSet1.addRow (new DataSetRow (vet, new double[]{2,2,2}));
			...
			while(linha4!=null){
				...
				if(indices.contentEquals("NDWI, EVI")==true){
					...
					testingSet.addRow (new DataSetRow (vetDescritores, new double[]{0,0,0}));
				}
				else if(indices.contentEquals("NDVI, EVI, NIR")==true){
	...
	//			int novaIntensidade = testNeuralNetwork(neuralNetwork1, testingSet);
				...
				imgsaolournecoRed[i][j]= (int) (255*netOut[0]);
				imgsaolournecoGreen[i][j]= (int) (255*netOut[1]);
				imgsaolournecoBlue[i][j]= (int) (255*netOut[2]);
	...
	public static double[] testNeuralNetwork(NeuralNetwork<BackPropagation> nnet, DataSet testSet) {
		...
		/*if(networkOutput[0]<=1)
			return 0;
		else if(networkOutput[0]>1 && networkOutput[0]<=2)
			return 125;
		else
			return 255;*/
