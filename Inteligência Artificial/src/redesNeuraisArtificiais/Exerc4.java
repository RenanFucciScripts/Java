...
public class Exerc4 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			double[][] w = new double[x1.length][x1[0].length];
			vetorPadroes.add(x1);
			...
			for (int i = 0; i < w.length; i++) {
				for (int j = 0; j < w[0].length; j++) {
					...
					if(i%2==0){
						w[i][j]=rand.nextDouble() * .5;
						System.out.println("Peso Rand: "+w[i]);
					}
					else{
						w[i][j]=rand.nextDouble() * -.5;
						System.out.println("Peso Rand: "+w[i]);
		...
		double[][] vetorExec={{0,0,0},
							  {0,1,0},
							  {0,0,0}};

		int x=execPerceptron(wfinal, vetorExec, limiar);
		if(x==1)
			System.out.println("classe: T");
		else
	...
	public static int execPerceptron(double[][] wfinal, double[][] vetorExec, double limiar){
		double u= arit.funçãoU(vetorExec, wfinal, limiar);
		return (int) u;
	}
}
