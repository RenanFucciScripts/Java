...
public class PuzzleAestrela{
	...
	public int contarDiferença(Noh no){
		...
		for (int i = 0; i < tmp.length; i++) {
			for (int j = 0; j < tmp.length; j++) {
				...
				tmp[i][j]=Math.abs(this.solução[i][j]-no.estado[i][j]);
				if(tmp[i][j]!=0){
					cont+=1;
				}
			}
	...
	*/
	...
	public Queue<Noh> ordenar(Queue<Noh> fila){
		...
		for (int i = 0; i < fila.size(); i++) {
			for (int j = 1; j < fila.size(); j++) {
				Noh no= lista.get(i);
				Noh no1= lista.get(j);
				//System.out.println("1º "+no.getEtiqueta()+" Custo "+no.getCustoCaminho());
				//System.out.println("2º "+no1.getEtiqueta()+" Custo "+no1.getCustoCaminho());
	...
	public Queue<Noh> expandir(Noh noh){
		...
		for (int i=0 ;i<loop; i++){
			...
			s.nohPai=noh;
			s.ação=("filho "+(i));
			s.custoCaminho=(contarDiferença(s)+(noh.profundidade+1));
